...
scibeautify <- function(.data,
  ...
  stopifnot(exprs = {
    ...
    sci_mode %in% c(NA, "on", "off", "auto")
    justify_mode %in% c(NA, "l", "c", "r", "d", "lx", "cx", "rx", "dx")
  })

  # adjust options
  if (is.na(sci_mode)) sci_mode <- "off"
  ...
  convert_sciformat <- function(x) {
    ...
    x <- gsub("([-\\d]+)$", exp_gsub, x, perl = T)

    base_gsub <- switch(output_format,
      ...
      "unicode" = switch(sci_format,
        "x" = paste0("\U2007\U00D7\U2007",base,"^","\\1"),
